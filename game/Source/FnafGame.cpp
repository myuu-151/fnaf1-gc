...
bool FnafGame::LoadData()
{
    ...
    auto loadImage = [this](const std::string& name) -> bool
    {
        return ReadDataFile("img/" + name + ".jpg", mImages[name]);
    };
    ...
    bool ok = true;
    for (const char* name : kBackgrounds)
    {
        ok = loadImage(name) && ok;
    }
    ...
    for (int32_t i = 0; i < (int32_t)mFlipFrames.size(); ++i)
    {
        ...
        ok = LoadSprite(name, mFlipFrames[i]) && ok;
    }
    ...
    for (int32_t side = 0; side < 2; ++side)
    {
        ...
        for (int32_t i = 0; i < (int32_t)mDoors[side].mFrames.size(); ++i)
        {
            ...
            ok = LoadSprite(name, mDoors[side].mFrames[i]) && ok;
        }
    }
    ...
    for (int32_t closed = 0; closed < 2; ++closed)
    {
        for (int32_t light = 0; light < 2; ++light)
        {
            ...
            ok = LoadSprite(name, mButtons[closed][light]) && ok;
        }
    }

    for (const char* sound : { "fan", "light", "door", "blip", "tablet", "scream", "windowscare", "chimes", "steps", "powerdown" })
    {
        SoundWave* wave = LoadPcmSound(std::string("snd/") + sound + ".pcm", 22050);
        if (wave != nullptr)
        {
            mSounds[sound] = wave;
        }
    ...
    if (mDoors[0].mFrames.empty() || mDoors[1].mFrames.empty() || mFlipFrames.empty())
    {
        LogError("FNAF1: manifest is missing sprite counts");
        ok = false;
    }
...
bool FnafGame::AnyAt(Room room) const
{
    return mBonnie.mRoom == room || mChica.mRoom == room;
}
